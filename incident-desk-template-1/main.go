...
// db is the shared connection pool. It stays nil until a database is bound and
// reachable — every handler is written to work with db == nil so the app runs
// (and renders its first-run banner) before the reader has attached Postgres.
var db *sql.DB

// appTitle is the display name shown in the UI header. Optional; defaults below.
var appTitle = "Lumen"
...
func main() {
	...
	// Connect best-effort. A missing or unreachable database is not fatal:
	// the app boots, /api/health reports "database":"absent", and the UI shows
	// a friendly first-run banner instead of an error. This is what makes the
	// no-database state a genuine, honest first stop for the reader.
	if conn, err := connectDB(); err != nil {
		log.Printf("no database bound (%v) — starting in first-run mode", err)
	} else {
		db = conn
		if err := migrateAndSeed(db); err != nil {
			log.Printf("warn: schema migrate/seed failed: %v", err)
		} else {
			log.Printf("database connected — schema ready")
		}
	}
