...
- **Go:** Uses `godotenv` for optional `.env`. `WORKFLOW_EXEC_PATH` is required for expense extraction; `GATEWAY_URL` defaults to `http://localhost:3456`; `PORT` defaults to 80 (8210 in dev).
...
- **Example deploy command:**
  ```
  dibbla deploy . --alias <name> --require-login --google-scopes https://www.googleapis.com/auth/spreadsheets -e "WORKFLOW_EXEC_PATH=$WORKFLOW_EXEC_PATH"
  ```
