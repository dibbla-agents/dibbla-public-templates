Set up a fresh Dibbla "getting-started" project in this directory and
...
3. Install the getting-started template.
   ...
   After the pipeline finishes, verify the backend responds:
     curl -s http://localhost:8090 || echo "backend not reachable"
...
4. Write the pre-deploy gate artefacts, THEN deploy.
   ...
   Then deploy from the template subdirectory (getting-started-1/):
     dibbla deploy
   ...
   until you find a free one:
     dibbla deploy --alias getting-started1
     dibbla deploy --alias getting-started2
   and so on.
