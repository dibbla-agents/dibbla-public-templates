...
- **Vite uses `strictPort`.** The dev server fails loudly if 5185 is taken rather than silently switching, so the URL the task opens is always the one Vite is on.
...
- **Starting servers by hand:** `PORT=8090 go run main.go` and `PORT=8090 VITE_PORT=5185 npm run dev` in `frontend/` — both processes need `PORT`, or the proxy points at the wrong place.
...
- **Local Vite proxy:** The frontend proxies `/api` to the backend port from `.dev/backend.port` (or `PORT` env, default 8090). The dibbla task sets `PORT=8090` for both processes. When running them manually, `PORT` must be set for the backend *and* for `npm run dev` — otherwise the proxy points somewhere else and you get "http proxy error: /api/...".
