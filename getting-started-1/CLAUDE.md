...
- **Backend:** Go 1.23+, Go Fiber v2 — serves embedded static files and API.
...
- **Go:** Uses `godotenv` for optional `.env`. `PORT` defaults to 80 (8090 in dev — set by the Dibbla task).
...
- **Dev:** Backend on 8090, Vite dev server on 5185 with API proxy to backend.
...
- **Backend publishes its actual port.** On startup, `main.go` writes the bound port and PID to `.dev/backend.port` and `.dev/backend.pid`. These files are cleaned up on shutdown (SIGINT/SIGTERM). If the preferred port (8090) is taken, the backend falls back to 8091, 8092, … up to 20 tries.
- **Vite proxy reads `.dev/backend.port` dynamically.** `frontend/vite.config.ts` uses a `router` callback so `/api` always forwards to the backend's real port, even if it fell back. `strictPort: true` on Vite means the dev server fails loudly if 5185 is taken (no silent switching).
...
- **Do not hardcode `8090` on the frontend.** Use relative `/api/...` paths (Vite proxies them). If you need the backend URL in a script, read `.dev/backend.port`.
...
**Do not deploy unless the user explicitly asks.** After making changes, assume the user wants to test locally first. Point them to the local dev servers (backend on port 8090, Vite on port 5185) and remind them to hard-reload the browser (Cmd+Shift+R / Ctrl+Shift+R) to bypass the cache if frontend changes aren't showing.
...
- **Local Vite proxy:** The frontend proxies `/api` to the backend port from `.dev/backend.port` (or `PORT` env, default 8090). The dibbla task sets `PORT=8090` and the proxy self-heals if the backend falls back. When running backend manually, use `PORT=8090 go run main.go`.
